...
- `VITE_CLERK_PUBLISHABLE_KEY`: Clerk publishable key used by the frontend auth UI
  ...
  When running in Docker Compose, `VITE_CLERK_PUBLISHABLE_KEY` is read from `infrastructure/.env` (copy `infrastructure/.env.example` first, then replace the placeholder with your real Clerk publishable key).
